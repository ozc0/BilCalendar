...
import library.User;
/**
 * Class representing a personal event fragment.
 *
 * @author Team of Ministler
 * @date 12/25/2019
 */
public class HostPersonalEventFragment extends Fragment implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
    private static final String TAG = "HostPersonalEventFragment";

    private TextInputLayout eventName, notes;
    private TextView dateText, endTimeText, addRepetitiveText, startTimeText;
    private LinearLayout dateLayout, startLayout, endLayout, addRepetitiveLayout;

    private FloatingActionButton hostEventButton;

    private int layoutSelection;
    private int addRepetitiveSelection = 0;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.host_personal_event,container,false);

        eventName = view.findViewById(R.id.personal_event_name_textInput);
        notes = view.findViewById(R.id.personal_event_notes_textInput);

        dateText =  view.findViewById(R.id.personal_event_date_textView);
        startTimeText = view.findViewById(R.id.personal_event_start_time_textView);
        endTimeText = view.findViewById(R.id.personal_event_end_time_textView);

        addRepetitiveText =  view.findViewById(R.id.personal_event_add_repetitive_textView);

        dateLayout = view.findViewById(R.id.personal_event_date_layout);
        startLayout = view.findViewById(R.id.personal_event_start_time_layout);
        endLayout = view.findViewById(R.id.personal_event_end_time_layout);
        addRepetitiveLayout = view.findViewById(R.id.personal_event_add_repetitive_layout);

        calendarStartDate = android.icu.util.Calendar.getInstance();
        calendarEndDate = android.icu.util.Calendar.getInstance();

        addRepetitiveSelection = 0;

        setDateText();
        setLayoutListeners();

        hostEventButton = view.findViewById(R.id.host_personal_event_button);
        hostEventButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if( isAppropriate() ) {
                    ...
                    databaseManager.addPersonalEvent( eventName.getEditText().getText().toString(), notes.getEditText().getText().toString(), user.getId(), eventDay, startHour, startMinute, endHour, endMinute, addRepetitiveSelection );
    ...
    private void setDateText() {
        ...
        /*startTimeText.setText(sdf.format(calendarStartDate.getTime()));
        endTimeText.setText(sdf.format(calendarEndDate.getTime()));*/
    }
    ...
    private void setLayoutListeners() {
        ...
        addRepetitiveLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                layoutSelection = 3;
                startFeedBackAnimation( addRepetitiveLayout );

                AlertDialog.Builder builder = new AlertDialog.Builder(getContext(), R.style.AlertDialogStyle);

                builder.setTitle("Select Repetitiveness");

                String[] repetitiveSelectables = {"One Time Event", "Weekly Repetitive Event", "Monthly Repetitive Event"};

                builder.setItems(repetitiveSelectables, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        switch (which) {
                            case 0: // One Time Event
                                addRepetitiveText.setText("One Time Event");
                                addRepetitiveSelection = 0;
                                break;
                            case 1: // Weekly Repetitive Event
                                addRepetitiveText.setText("Weekly Repetitive Event");
                                addRepetitiveSelection = 1;
                                break;
                            case 2: // Monthly Repetitive Event
                                addRepetitiveText.setText("Monthly Repetitive Event");
                                addRepetitiveSelection = 2;
                                break;
                        }
                    }
                });
